    ...
    var body: some View {
        ...
        VStack {
            if meetingViewController.participants.count == 0 {
                ...
                VStack {
                    VStack(spacing: 20) {
                        ...
                        List {
                            ForEach(
                                meetingViewController.participants.indices,
                                id: \.self
                            ) { index in
                                Text(
                                    "Participant Name: \(meetingViewController.participants[index].displayName)"
                                )
                                ZStack {

                                    ParticipantView(
                                        track:
                                            meetingViewController.participants[
                                                index
                                            ].streams.first(where: {
                                                $1.kind == .state(value: .video)
                                            })?.value.track as? RTCVideoTrack
                                    ).frame(height: 250)
                                    if meetingViewController.participants[index]
                                        .streams.first(where: {
                                            $1.kind == .state(value: .video)
                                        }) == nil
                                    {
                                        Color.white.opacity(1.0).frame(
                        ...
                        HStack {
                            ...
                            Button {
                                if isScreenShare {
                                    isScreenShare = false
                                    Task {
                                        await meetingViewController.meeting?
                                            .disableScreenShare()
                                    }
                                } else {
                                    isScreenShare = true
            ...
            if meetingId?.isEmpty == false {
                meetingViewController.joinMeeting(
                    meetingId: meetingId!,
                    userName: userName!
                )
            } else {
